...
INTENT_SYSTEM_PROMPT = """당신은 여행 메이트 게임의 의도 분류기입니다.
...
tool 의도 시 params에서 IATA 코드로 공항을 추출하세요. 날짜를 명시하지 않으면 date를 null로 두세요.
주요 공항 IATA: 인천=ICN, 김포=GMP, 도쿄(나리타)=NRT, 도쿄(하네다)=HND, 오사카=KIX, 파리=CDG, 런던=LHR, 방콕=BKK, 뉴욕=JFK, 싱가포르=SIN

응답 형식 (JSON only):
{
  "intent": "dialogue" | "tool" | "selection",
  "params": {
    // tool인 경우: {"origin": "ICN", "destination": "NRT", "date": "2025-08-01" or null, "adults": 1}
...
→ {"intent": "tool", "params": {"origin": "ICN", "destination": "NRT", "date": null, "adults": 1}, "reason": "항공편 검색 요청"}

사용자: "8월 15일에 인천에서 파리 가고 싶어"
→ {"intent": "tool", "params": {"origin": "ICN", "destination": "CDG", "date": "2025-08-15", "adults": 1}, "reason": "출발지/목적지/날짜 모두 명시된 항공편 요청"}
